...
import numpy as np
from utils.drawables import DirectionalLight, PointLight, SpotLight, Texture
...
class SceneGraph:
    ...
    def get_transform(self, node):
        ...
        rotation_matrix = (
            tr.rotationY(node["rotation"][1])
            @ tr.rotationX(node["rotation"][0])
            @ tr.rotationZ(node["rotation"][2])
        )
    ...
    def update(self):
        ...
        for src, dst in edges:
            if not dst in self.transformations:
                self.transformations[dst] = self.transformations[
                    src
                ] @ self.get_transform(dst)
    ...
    def draw(self):
        ...
        for src, dst in edges:
                ...
                for pipeline in current_pipelines:
                    ...
                    if isinstance(current_node["light"], DirectionalLight):
                        if "u_dirLight.direction" in pipeline.uniforms:
                            pipeline["u_dirLight.direction"] = (
                                self.transformations[src] @ self.get_forward(dst)
                            )[:3]
                            pipeline["u_dirLight.ambient"] = current_node[
                                "light"
                            ].ambient
                            pipeline["u_dirLight.diffuse"] = current_node[
                                "light"
                            ].diffuse
                            pipeline["u_dirLight.specular"] = current_node[
                                "light"
                            ].specular
                        ...
                        if "u_numPointLights" in pipeline.uniforms:
                            ...
                            pipeline[
                                f"u_pointLights[{str(pointLightIndex)}].position"
                            ] = position
                            pipeline[
                                f"u_pointLights[{str(pointLightIndex)}].ambient"
                            ] = current_node["light"].ambient
                            pipeline[
                                f"u_pointLights[{str(pointLightIndex)}].diffuse"
                            ] = current_node["light"].diffuse
                            pipeline[
                                f"u_pointLights[{str(pointLightIndex)}].specular"
                            ] = current_node["light"].specular
                            pipeline[
                                f"u_pointLights[{str(pointLightIndex)}].constant"
                            ] = current_node["light"].constant
                            pipeline[
                                f"u_pointLights[{str(pointLightIndex)}].linear"
                            ] = current_node["light"].linear
                            pipeline[
                                f"u_pointLights[{str(pointLightIndex)}].quadratic"
                            ] = current_node["light"].quadratic
                        ...
                        if "u_numSpotLights" in pipeline.uniforms:
                            ...
                            pipeline[
                                f"u_spotLights[{str(spotLightIndex)}].position"
                            ] = position
                            pipeline[
                                f"u_spotLights[{str(spotLightIndex)}].direction"
                            ] = (self.transformations[src] @ self.get_forward(dst))[:3]
                            pipeline[f"u_spotLights[{str(spotLightIndex)}].ambient"] = (
                                current_node["light"].ambient
                            )
                            pipeline[f"u_spotLights[{str(spotLightIndex)}].diffuse"] = (
                                current_node["light"].diffuse
                            )
                            pipeline[
                                f"u_spotLights[{str(spotLightIndex)}].specular"
                            ] = current_node["light"].specular
                            pipeline[
                                f"u_spotLights[{str(spotLightIndex)}].constant"
                            ] = current_node["light"].constant
                            pipeline[f"u_spotLights[{str(spotLightIndex)}].linear"] = (
                                current_node["light"].linear
                            )
                            pipeline[
                                f"u_spotLights[{str(spotLightIndex)}].quadratic"
                            ] = current_node["light"].quadratic
                            pipeline[f"u_spotLights[{str(spotLightIndex)}].cutOff"] = (
                                current_node["light"].cutOff
                            )
                            pipeline[
                                f"u_spotLights[{str(spotLightIndex)}].outerCutOff"
                            ] = current_node["light"].outerCutOff
            ...
            if current_node["mesh"] is not None:
                ...
                textured = (
                    "u_texture" in current_pipeline.uniforms
                    and current_node["texture"] is not None
                )
                if textured:
                ...
                Setup de Mesh
                """
                current_pipeline["u_model"] = np.reshape(
                    self.transformations[dst], (16, 1), order="F"
                )
                current_node["mesh"].draw(
                    current_node["mode"], current_node["cull_face"]
                )
